...
public:
	...
	static void loadMIMETypes(map<string, string> &mimeTypesMap)
	{
		...
		cout << endl
			 << "Successfully Loaded the mime types" << endl;
	}
};
...
public:
	...
	{
		if (this->staticResourseFolderpath.length() == 0)
			return false;
		if (!FileSystemUtility::directoryExists(this->staticResourseFolderpath.c_str()))
			return false;
		...
		while (bytesLeftToRead > 0)
		{
			...
			if (feof(f))
				break; // we never reach to this condition
